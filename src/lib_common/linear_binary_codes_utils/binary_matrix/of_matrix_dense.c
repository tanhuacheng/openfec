...
#else
	printf ("m->row=%p\n", m->row);
	printf (" m->row size = %u\n", (unsigned int)(m->n_rows*sizeof *m->row));
#endif
	printf ("m->bits=%p\n", m->bits);
	printf (" m->bits size = %u\n", (unsigned int)(m->n_words*m->n_rows*sizeof *m->bits));
	printf ("sizeof(mod2word)=%u\n", (unsigned int)sizeof (of_mod2word));
	OF_EXIT_FUNCTION
}
